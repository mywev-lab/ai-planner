  ...
  return (
    <section className="panel p-5">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-bold">🌅 Resumo {range === "day" ? "do Dia" : "da Semana"}</h2>
      ...
      {loading && <p style={{ color: "var(--muted)" }}>Analisando sua agenda…</p>}
        ...
        <>
          ...
          <div className="mt-3 flex gap-2 flex-wrap">
            <span className="badge" style={{ color: data.connected.calendar ? "var(--ok)" : "var(--muted)" }}>
              Agenda {data.connected.calendar ? "conectada" : "desligada"}
            </span>
            <span className="badge" style={{ color: data.connected.tasks ? "var(--ok)" : "var(--muted)" }}>
              Tarefas {data.connected.tasks ? "ativas" : "desligadas"}
            </span>
          </div>
        </>
      )}
